...
## Why it matters here
[[Karpathy - LLM Wiki]] names it as the obvious tool to add **once the wiki outgrows [[Index and Log|the index file]]** (~100+ sources). It exposes both a **CLI** (the LLM can shell out to it) and an **MCP server** (native tool use) — so it slots on top of the existing Markdown without changing the substrate.
