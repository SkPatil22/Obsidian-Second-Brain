...
**The wiki answer:** the synthesis is a durable artifact — a [[Compounding Knowledge Artifact]], built and maintained via the [[Three-Layer Architecture]] (raw sources → wiki layer → schema) through the [[Ingest Query Lint|Ingest]] step — compiled once, not re-derived per query. You read it (in [[Obsidian]]); the LLM writes it.
