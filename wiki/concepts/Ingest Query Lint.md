...
## Ingest
Drop a source into `.raw/`, tell the LLM to process it. It reads, discusses takeaways, writes a [[sources/_index|source summary]] page, updates the index, updates relevant [[entities/_index|entity]] and [[concepts/_index|concept]] pages, and appends to the log. **A single source touches ~10–15 pages.** Default workflow (Karpathy's preference): one source at a time, stay involved, read the summaries. Batch ingest is possible with less supervision. Data moves from the raw layer upward through [[Three-Layer Architecture]], building the [[Compounding Knowledge Artifact]] with each pass. This compile-at-ingest approach is what puts the wiki firmly on the wiki side of the [[Wiki vs RAG]] spectrum: knowledge is written once, not re-derived on every query.

## Query
Ask questions against the wiki. The LLM reads the index first, drills into relevant pages, and answers **with citations**. Key insight: **good answers get filed back** as new pages (a comparison, an analysis, a discovered connection) so explorations compound into the [[Compounding Knowledge Artifact]] instead of vanishing into chat history. At scale (100+ sources), [[qmd]] augments this step with BM25+vector search so retrieval stays fast without changing the wiki substrate.
